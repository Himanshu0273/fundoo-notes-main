...
async def create_user(request: user_schema.User, db: Session = Depends(get_db)):
    ...
    try:
        ...
        verify_token = token_obj.create_access_token(
            {"sub": new_user.username}, expires_delta=timedelta(minutes=10)
        )
...
def verify_email(token: str, db: Session = Depends(get_db)):
    try:
        ...
        if user.is_verfied:
            return HTMLResponse(
                content="<h2>Email already verified</h2>", status_code=200
            )
        ...
        return HTMLResponse(
            content="<h2>Email verified successfully!</h2>", status_code=200
        )

    except JWTError:
        return HTMLResponse(
            content="<h2>Invalid or expired token</h2>", status_code=401
        )
...
def update_details(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Internal Server Error during user update"
        )
...
def delete_user(
    id: int, db: Session = Depends(get_db), current_user=Depends(get_user_with_headers)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Internal Server Error during user deletion"
        )
